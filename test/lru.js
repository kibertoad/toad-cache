...
describe("Testing functionality", function () {
  ...
  it("It should delete", function () {
    ...
    assert.strictEqual(this.cache.size, 4, "Should be '4'");
    assert.strictEqual(
      this.cache.items.get("e").next,
      null,
      "Should be 'null'"
    );
    assert.strictEqual(
      this.cache.items.get("e").prev.key,
      "d",
      "Should be 'd'"
    );
    assert.strictEqual(
      this.cache.items.get("d").next.key,
      "e",
      "Should be 'e'"
    );
    assert.strictEqual(
      this.cache.items.get("d").prev.key,
      "c",
      "Should be 'c'"
    );
    assert.strictEqual(
      this.cache.items.get("c").next.key,
      "d",
      "Should be 'd'"
    );
    assert.strictEqual(
      this.cache.items.get("c").prev.key,
      "b",
      "Should be 'b'"
    );
    assert.strictEqual(
      this.cache.items.get("b").next.key,
      "c",
      "Should be 'c'"
    );
    assert.strictEqual(
      this.cache.items.get("b").prev,
      null,
      "Should be 'null'"
    );
    this.cache.delete("c");
  ...
  it("It should expose expiration time", function () {
    ...
    assert.strictEqual(
      typeof this.cache.expiresAt(this.items[0]),
      "number",
      "Should be a number"
    );
    assert.strictEqual(
      this.cache.expiresAt("invalid"),
      undefined,
      "Should be undefined"
    );
  });
  ...
  it("It should reset the TTL with optional parameter", function (done) {
    ...
    setTimeout(() => {
      ...
      assert.strictEqual(
        n2 > n1,
        true,
        "Should be greater than first expiration timestamp"
      );
      done();
    }, 11);
  });
